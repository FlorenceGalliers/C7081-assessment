...
data$if_basement <- as.factor(data$if_basement) # convert basement to factor
data$if_renovated <- as.factor(data$if_renovated) # convert to factor
data$city <- as.factor(data$city) # convert to factor
...
# 8.0 Trialling Linear Regression with houses only up to $1,000,000. ####
...
# 14.0 Trialling Linear Regression without Location variables ####
new_train <- train[, -11]
new_test <- test[, -11]
